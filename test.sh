#!/bin/bash
# Compile, then run all ProgramBench test branches for kaushiksrini__parqeye.8072121.
#
...
INSTANCE=kaushiksrini__parqeye.8072121
...
# Only macOS needs the script shim: BSD `script` lacks -c. GNU script (Linux)
# already accepts -c, so leave the real binary on PATH there.
...
while read -r branch; do
    ...
    if [ ! -f "$tar" ]; then
        for _a in 1 2 3 4 5 6; do
            curl -fsSL -o "$tar" "$BASE_URL/$branch.tar.gz" && tar tzf "$tar" >/dev/null 2>&1 && break
            rm -f "$tar"; sleep $((_a * 10))
        done
    fi
    ...
    # Some test tarballs ship a broken `executable` symlink; rm it first so cp
    # writes a real binary instead of following the dead symlink.
    ...
    sed -i.pbbak -e 's/--timeout-method=thread/--timeout-method=signal/g' \
        -e 's/pip3 install --user/pip3 install/g' -e 's/pip install --user/pip install/g' \
